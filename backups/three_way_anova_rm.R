...
#-nortest for normality.  Shapiro Wilks doesnt work on values higher than 5000
library(nortest)
ad.test(palouse_sumloss_aggregate$sqroot_loss)$p.value
...
#palouse_sumloss_aggregate_sqroot <- aggregate(sqroot_loss ~ damagecause + year + county + commodity, palouse_sumloss, mean)

palouse_sumloss_aggregate_sqroot <- palouse_sumloss_aggregate[-c(5:13)] 

#AOV with error term
#factor include: year (within), damagecause (between), and commodity (between). County represents the subject.  Each
#county has a set of means for a range of commodities, with differing damage causes, for a range of years.

xx <- subset(palouse_sumloss_aggregate_sqroot, commodity == "WHEAT")
...
model.a <- lmer(sqroot_loss ~ damagecause*year + (1 | county), data=xxx)
...
ezANOVA(data=xxx, 
        ...
        wid=.(county), 
        within=.(year), 
        between = .(commodity, damagecause),
        type = 3) 



form <- lm(sqroot_loss ~ county*damagecause*commodity, data = palouse_sumloss_aggregate)
fit = Anova(form, )
